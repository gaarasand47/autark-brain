...
interface HealthEvidence { schemaVersion:string; correlationId:string; provenance:{source:string; sourceVersion:string}; artifactHash:string; observedAt:string; errorRate:number; latencyMs:number; cpuPercent:number; memoryBytes:number; status:'HEALTHY'|'DEGRADED'|'UNAVAILABLE'; evidenceHash:string; stateHash:string; replayKey:string; }
interface CredentialLease { schemaVersion:string; correlationId:string; provenance:{issuer:string; issuerVersion:string}; leaseId:string; environment:Environment; artifactHash:string; capability:string; issuedAt:string; expiresAt:string; revokedAt:string|null; stateHash:string; replayKey:string; }
```
